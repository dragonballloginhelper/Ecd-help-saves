...
`;
    ...
    const variableNames = ['_g', '_x', '_y', '_z', '_env', '_cache', '_val', '_dat'];

    for (let i = 0; i < targetJunkCount; i++) {
        const randVar = variableNames[Math.floor(Math.random() * variableNames.length)] + Math.floor(Math.random() * 10000);
        const randChar = chars[Math.floor(Math.random() * chars.length)];
        const op = Math.random();

        if (i % 2 === 0) {
            if (op < 0.33) {
                junkHeader += `local ${randVar} = "${randChar}" .. "${Math.floor(Math.random() * 9999)}"\n`;
            } else if (op < 0.66) {
                junkHeader += `local function ${randVar}() return ${Math.random() * 100} * ${Math.random() * 10} end\n`;
            } else {
                junkHeader += `pcall(function() local ${randVar} = {${Math.random()}, ${Math.random()}} end)\n`;
            }
        } else {
            if (op < 0.33) {
                junkFooter += `local ${randVar} = "${randChar}" .. "${Math.floor(Math.random() * 9999)}"\n`;
            } else if (op < 0.66) {
                junkFooter += `local function ${randVar}() return ${Math.random() * 100} * ${Math.random() * 10} end\n`;
            } else {
                junkFooter += `pcall(function() local ${randVar} = {${Math.random()}, ${Math.random()}} end)\n`;
            }
        }
    }
